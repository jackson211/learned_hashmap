    ...
    public:
                              ...
                              const std::vector<int> &breaks)
        {
            ...
            for (int i = 0; i < breaks.size() - 1; i++)
            {
                ...
                auto segment_train_x = std::vector<T>(
                    train_x.begin() + break_start, train_x.begin() + break_end);
                auto segment_train_y = std::vector<T>(
                    train_y.begin() + break_start, train_y.begin() + break_end);
                const T max_v = *std::max_element(segment_train_x.begin(),
                                                  segment_train_x.end());
                const T min_v = *std::min_element(segment_train_x.begin(),
                                                  segment_train_x.end());
